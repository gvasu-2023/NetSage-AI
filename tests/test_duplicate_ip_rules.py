from rules.duplicate_ip_rules import (
    diagnose_duplicate_ip_address
)


def test_duplicate_ip_address_detected():

    case_data = {
        "case_id": "CASE013",
        "title": "Duplicate IP Address",
        "category": "IP Configuration",

        "topology": {
            "faulty_device": "PC3",
            "conflicting_device": "PC1",
            "affected_network": "192.168.10.0/24",
            "default_gateway": "192.168.10.1"
        },

        "ip_configuration": {
            "duplicate_ip_detected": True,
            "pc1_ip_address": "192.168.10.10",
            "pc3_ip_address": "192.168.10.10",
            "duplicate_ip_address": "192.168.10.10",
            "expected_pc3_ip": "192.168.10.12"
        },

        "symptoms": {
            "ip_conflict_detected": True,
            "gateway_ping": "failed",
            "remote_ping": "failed"
        }
    }

    result = diagnose_duplicate_ip_address(case_data)

    assert result["fault_detected"] is True
    assert result["category"] == "IP Configuration"
    assert result["diagnosis"] == "Duplicate IP Address"
    assert result["faulty_device"] == "PC3"
    assert result["conflicting_device"] == "PC1"
    assert result["duplicate_ip_address"] == "192.168.10.10"
    assert result["expected_ip_address"] == "192.168.10.12"
    assert result["osi_layer"] == "Layer 3"
    assert result["confidence"] == "High"